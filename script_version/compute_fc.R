...
tryCatch({
	...
	exp_data <- myInputs$exp_data$value
	pheno_data <- myInputs$pheno_data$value
	dose_index <- myInputs$doseIDCol$value
	time_index <- myInputs$timeIDCol$value
	
	fc_data = TinderMIX::compute_fc(exp_data = exp_data,pheno_data = pheno,dose_index = dose_index,time_index = time_index)
